...
| `WARM_USERNAMES` | No | Comma-separated GitHub usernames to pre-warm in the background (e.g. `enoquesousa`). The service silently refreshes their data before the cache expires, eliminating cold-start latency for every card embed. |
